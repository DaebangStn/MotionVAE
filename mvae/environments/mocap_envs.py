...
class EnvBase(gym.Env):
    def __init__(
        ...
        indices = self.np_random.integers(0, self.mocap_data.shape[0], self.num_parallel)
...
class RandomWalkEnv(EnvBase):
    ...
    def reset(self, indices=None, seed=None, options=None):
        self.timestep = 0
        self.substep = 0
    ...
    def dump_additional_render_data(self):
        from mvae.common.misc_utils import POSE_CSV_HEADER
...
class HumanMazeEnv(EnvBase):
    ...
    def create_simulation_world(self):
        ...
        if self.is_rendered:
            from mvae.common.bullet_objects import Rectangle, VSphere
